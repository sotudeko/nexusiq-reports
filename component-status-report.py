...
def getPolicyViolationsForOverrideApplications():
  ...
  with open(appPolicyViolationsCsvFile, 'w') as fd:
    fd.write(
      'ApplicationName,ApplicationId,PackageUrl,ComponentHash,PolicyName,PolicyId,PolicyThreatCategory,PolicyThreatLevel,PolicyViolationId,Waived\n')
    fd.close()
    ...
    for component in components:
      ...
      for violation in violations:
        ...
        if policyThreatLevel >= 7:
          line = applicationName + "," + applicationId + "," + packageUrl + "," + componentHash + "," + policyName + "," + policyId + "," + policyThreatCategory + "," + str(
            policyThreatLevel) + "," + policyViolationId + "," + str(waived) + "\n"
      ...
      if type(component["securityData"]) is dict:
        ...
        if len(securityIssues) > 0:
          for securityIssue in securityIssues:
            ...
            line = applicationId + "," + hash + "," + packageUrl + "," + securityIssue["reference"] + "," + str(
              securityIssue["severity"]) + "," + securityIssue["status"] + "," + licenseStatus + "\n"
...
def makeStatusSummary():
  with open(statusSummaryCsvFile, 'w') as fd:
    fd.write(
      'ApplicationName,ApplicationId,OverrideStatus,Comment,PackageUrl,ComponentHash,CVE,SecurityScore,VulnStatus,LicenseStatus,PolicyName,PolicyThreatLevel,Waived,PolicyId,PolicyViolationId\n')
